...
def app():
    ...
    with st_spinner("正在获取和处理数据，请稍候..."):
        ...
        filtered_df = trade_date_df[mask]

        # 生成交易日列表并格式化为%Y%m%d
        trading_days = [date.strftime('%Y%m%d') for date in filtered_df['date']]
