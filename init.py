...
def create_baostock_code_hist_table():
    try:
        table_name = tbs.TABLE_CN_BAOSTOCK_CODE_MAP['name']
        ...
        if rows:
            ...
            start_date_str, end_date_str = get_recent_trade_range(date, 30)
            for row in rows:
                name, code, baostock_mapped_code = row
                print(row)
                # 获取30日历史数据
                hist_df = get_history_k_data(baostock_mapped_code, start_date_str, end_date_str)
...
if __name__ == "__main__":
    ...
    create_baostock_code_hist_table()
