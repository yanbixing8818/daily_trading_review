...
def prev_daily_limit(fname):
    df = pd.read_excel(fname)
    col_tag = df.columns.tolist()
    col_tag_tmp = []
    col_tag_sel = ['股票代码', '股票简称', '连续涨停天数', '涨停开板次数', '最终涨停时间', '涨停原因类别', \
                   '收盘价', '开盘价', '最高价', '涨停类型']
    col_tag_out = ['股票代码', '股票简称', '连续涨停天数', '涨停开板次数', '最终涨停时间', '涨停原因类别', \
                   '涨停类型', '赚钱效应', '亏钱效应', '涨停类别0']
    daily_limit_style = []
    ...
    xlsx_writer = pd.ExcelWriter('../../昨日涨停.xlsx', engine='openpyxl')
...
def prev_daily_limit_concat():
    df_01 = pd.read_excel('../../复盘统计_昨日涨停.xlsx', engine='openpyxl', sheet_name='昨日涨停')
    df_02 = pd.read_excel('../../昨日涨停.xlsx', engine='openpyxl', sheet_name='昨日涨停')
    df_out = pd.concat([df_01, df_02], ignore_index=True)

    xlsx_writer = pd.ExcelWriter('../../复盘统计_昨日涨停.xlsx', engine='openpyxl')
