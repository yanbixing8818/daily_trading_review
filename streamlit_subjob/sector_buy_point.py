# 板块买点计算，该文件依赖于：plate_rps_history.csv文件，需要执行：
# python -m tdx_rps_subjob.tdx_bankuaifenxi
# 来生成。
...
# 添加父目录到路径，以便导入tdx_rps_subjob模块
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from tdx_rps_subjob.tdx_bankuaifenxi import check_plate_buy_point1, check_plate_buy_point2, check_plate_buy_point3
...
def load_plate_rps_history():
    # 修改路径以适配新的目录结构
    # 从streamlit_subjob目录访问tdx_rps_subjob目录
    current_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(current_dir)
    path = os.path.join(parent_dir, 'tdx_rps_subjob', 'plate_rps_history.csv')
    
    if not os.path.exists(path):
        st.error('plate_rps_history.csv 文件不存在，请先运行主分析脚本生成数据。')
        st.info('请运行: python -m tdx_rps_subjob.tdx_bankuaifenxi')
        st.info(f'当前查找路径: {path}')
        return None
