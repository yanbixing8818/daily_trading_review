...
@st.cache_data(ttl=3600, show_spinner=False)
def process_data(trade_dates):
    limit_up_counts, limit_down_counts = get_limit_up_down_stocks(trade_dates)
    result_df = pd.DataFrame(limit_up_counts, columns=['日期', '涨停数量'])
    result_df = pd.merge(result_df, pd.DataFrame(limit_down_counts, columns=['日期', '跌停数量']), on='日期')
    return result_df

def draw_limit_up_down_plot(result_df):
...
def app():
    trade_dates = tuple(get_trade_dates_last_n(15))  # 强制转为tuple以保证缓存生效
    result_df = process_data(trade_dates)
    plt_obj = draw_limit_up_down_plot(result_df)
    st.pyplot(plt_obj)
    plt.close()
