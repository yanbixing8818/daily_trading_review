...
def send_zhangdietingshuliang_to_dingtalk():
    ...
    plt.title('近15个交易日涨跌停数量变化')
    plt.xlabel('日期')
    plt.ylabel('数量')
    plt.xticks(ticks=range(len(result_df)), labels=result_df['日期'], rotation=45)
    plt.legend()
    plt.grid(axis='x', linestyle='--', color='gray', alpha=0.5)  # 添加竖线
    plt.tight_layout()
    # 标注数值
    for i, row in result_df.iterrows():
        plt.text(i, row['涨停数量'], str(row['涨停数量']), ha='center', va='bottom', fontsize=10, color='red')
        plt.text(i, row['跌停数量'], str(row['跌停数量']), ha='center', va='bottom', fontsize=10, color='green')
    buf = io.BytesIO()
